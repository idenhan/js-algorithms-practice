function solution(n, lost, reserve) {
  ...
  for(let i = 1 ; i <= n ; i++){
    ...
    if(lost.includes(i)){ // 체육복을 도둑 맞은 학생이라면
      setArr[i-1]-- // 1개를 뺀다.  주의 - 학생번호와index는다르다.
    }       
  }
  for(let i = 0 ; i < n ; i++){
    if(!setArr[i]){  // 0은 falsy 한 값이기 때문에 !를 붙여주면truthy한 값이 된다. (setArr[i]===0) 과 같다.
      if(setArr[i-1]===2){ // 앞에 있는 학생이 체육복을 2개를 가지고 있다면 
        setArr[i]++ //내꺼 하나 플러스
  ...
  for(let i = 0 ; i< n ; i ++){ // 체육복이 한개 이상인 학생들을answer에 추가한다.
    if(setArr[i] >= 1){
      answer++
    }
  }
